...
init1: # reading chars and storing to memory
        ...
        # FENCE POST PROBLEM -- execute loop instrucs once outside the loop to deal with empty input
	...
	addi t0, t0, 1
	#check whether first char is the enter key aka the input is empty
	beq a0, t1, init1
...
loadprint:  
	 ...
	 innerloop:
	 lb t2, 0(t0) #store value at t0 to register t2
	 beqz t2, incrementasciicounter # if char at counter == null char, restart going through string with next ascii char
